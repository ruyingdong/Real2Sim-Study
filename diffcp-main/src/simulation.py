from taichi_variable_definition import *
...
def p2g(f: ti.i32, grad_update: ti.i32):
    ...
    for p in range(0, n_vertex_particles):
        ...
        temp_v = vertex_v[f, p]
        temp_v = vertex_v[f, p] + [0, 0, dt * scene_scaling * gravity]
        for i in ti.static(range(3)):
            for j in ti.static(range(3)):
                for k in ti.static(range(3)):
                    offset = ti.Vector([i, j, k])
...
def initialize_objects():
    ...
    #print("grasp_point_pos shape:", grasp_point_pos.shape)
    #print("Using the first frame of grasp_point_pos for operator initialization.")
    
    # 打印当前帧的抓取点位置
    #print("grasp_point_pos[0]:", grasp_point_pos[0])
    #print("operator_model_without_position shape:", operator_model_without_position.shape)
    ...
    #print("After initialization, grasp_point_model shape:", grasp_point_model.shape)
    #print("grasp_point_model first 5 points:", grasp_point_model[:5])
