    ...
    for grad_updates in range(n_grad_updates):
        ...
        with ti.ad.Tape(loss=loss):
            ...
            for f in range(max_steps - 1):
                #print(f"grad_update {grad_updates}: f = {f}") 
                forward(f, grad_updates)
                
                # 每100个步骤保存一次
                if (f + 1) % 10 == 0:
                    garment_vertices = vertex_x.to_numpy()[f]
                    operator_pos = operator_x.to_numpy()[f]
                    
                    np.save(f'garment_vertices_step_{f+1}.npy', garment_vertices)
                    np.save(f'operator_pos_step_{f+1}.npy', operator_pos)
                    
                    print(f"Saved garment_vertices and operator_pos at step {f+1}")
                    print(f"Step {f+1}, first operator particle pos:", operator_pos[0])
            #print(f"Calling compute_loss with f = {f}")
            compute_loss(f)
        timeD = time.time()
