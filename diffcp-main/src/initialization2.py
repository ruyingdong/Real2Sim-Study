...
cloth_file = data_dir + '/bigbang.ply'
#cloth_file1 = data_dir + '/plane.ply'
print("New_cloth_file:", cloth_file)
#mark_file = data_dir + '/rigid_cube_top_face_center_position.npy'
world_target_path = data_dir + '/pointclouds750pink.npy'
target_pcd_array = np.load(world_target_path)
print("New pointcloud shape:", target_pcd_array)
#mark_pos = np.load(mark_file)
grasp_seq = np.load(data_dir + "1.npy")
#grasp_seq = np.load(data_dir + "trajectory_short.npy")
grasp_seq = grasp_seq.reshape(-1, 1, 3) 
print("New grasp_seq shape:", grasp_seq.shape)
dx, inv_dx = 1 / n_grid, float(n_grid)
initial_step = 1
n_sample_target_points = target_pcd_array.shape[1]
n_grad_updates = target_pcd_array.shape[0]
print("n_grad_updates", n_grad_updates)
...
print("end_frame is", n_actions)
grasp_point_num = grasp_seq[0].shape[0]
print("grasp_point_num:", grasp_point_num)
soft_body_mesh = o3d.io.read_triangle_mesh(cloth_file)
#soft_body_mesh1 = o3d.io.read_triangle_mesh(cloth_file1)
...
#soft_body_model_vertices1 = np.asarray(soft_body_mesh1.vertices) * scene_scaling
#soft_body_mesh1.vertices = o3d.utility.Vector3dVector(soft_body_model_vertices1)
#soft_body_model_triangles1 = np.asarray(soft_body_mesh1.triangles)

grasp_seq = grasp_seq * scene_scaling
#print(grasp_seq)
#cube_model = cube_model * scene_scaling
target_pcd_array = target_pcd_array * scene_scaling
#print(target_pcd_array)
garment_surface_area = soft_body_mesh.get_surface_area()

#garment_surface_area1 = soft_body_mesh1.get_surface_area()
...
#print("Adjusted n_operator_particles:", n_operator_particles)
...
#upper_bound = dx * (bound - 1)
lower_bound = dx * (2 - 1)
#lower_bound = 0.31
upper_bound = (1 - dx * (2 - 1))
#upper_bound = 4
print("lower:", lower_bound)
print("upper:", upper_bound)
points = [
    [lower_bound, lower_bound, -0.17],
    [upper_bound, lower_bound, -0.17],
    [lower_bound, upper_bound, -0.17],
    [upper_bound, upper_bound, -0.17],
...
#print("grasper_group_size:", grasper_group_size)
...
#print("newtargetpcdarray", target_pcd_array)
...
target_pcd.points = o3d.utility.Vector3dVector(target_pcd_array[15])  # 显示第0帧的目标点云
target_pcd.paint_uniform_color([0, 1, 0])  # 给目标点云上一个颜色(绿色)

# 计算法线和上色
soft_body_mesh.compute_vertex_normals()
soft_body_mesh.paint_uniform_color([0.7, 0.7, 0.7])  # 淡灰色

# 给边界框上色
line_set.paint_uniform_color([0, 0, 1])  # 蓝色

# 将所有几何体放在列表中
geometry_list = [soft_body_mesh, grasp_point_pos_pcd, target_pcd, line_set]
