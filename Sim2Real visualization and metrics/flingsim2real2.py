...
def run_visualization_videos(dirs, out_dir, total_steps, num_points, frame_rate=35):
    """
    整体流程：
      - 对每一步加载各来源点云（共 5 个 OBJ、3 个 TXT、1 个 PCD），并对齐
      - 为每个视图分别生成单帧图像（不计算指标），以及生成一个叠加视图帧
      - 将这 10 张图像（9 个单独视图 + 1 个叠加视图）排列成 2 行 5 列的复合帧
      - 将所有复合帧合成一个视频（最后一帧额外重复 3 秒）
    """
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    ...
    custom_colors = [
        ...
        (0.7, 0.7, 0.0, 1.0),   # TXT3
        (0.0, 0.0, 0.0, 1.0)    # PCD
    ]
    
    composite_frames = []  # 存储每一步生成的复合帧
    ...
    for step in range(1, total_steps + 1):
        ...
        step_images.append(overlaid_img)  # 此时共10张图
        
        # 将10张图排列成2行5列（假设所有图像大小一致）
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_dir', type=str, default='allflingbrown_videos', 
    ...
    run_visualization_videos(dirs, args.out_dir, args.steps, args.points, frame_rate=35)
