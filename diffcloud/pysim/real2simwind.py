...
with open('conf/rigidcloth/lift/d.json','r') as f:
	config = json.load(f)
...
def plot_pointclouds(pcls, title="", angle=-90):
    ...
    for i,(color,label,points) in enumerate(zip(colors,labels,pcls)):
        ...
        ax_both.view_init(90, angle)
        ax_curr.view_init(90, angle)
        ax_ref.view_init(90, angle)
        ax_initial.view_init(90, angle)
...
def get_loss_per_iter(sim, epoch, sim_step, demo_dir, save, initial_states=None):
    ...
    if (save):
        ...
        plot_pointclouds([curr_pcl_cloth_only, ref_pcl, initial_pcl], title='%s/fixed_epoch%02d-%03d'%(out_path,epoch,sim_step),angle=-90)
...
def run_sim(steps, sim, epoch, demo_dir, save, given_params=None, initial_states=None):
    if given_params is None:
        stretch_multiplier, mass_multiplier, bending_multiplier  = torch.sigmoid(param_g)
    else:
        stretch_multiplier, mass_multiplier, bending_multiplier  = given_params
    loss = 0.0
    ...
    orig_bending = sim.cloths[0].materials[0].bendingori
    new_bending_multiplier = scale(bending_multiplier, low_bending, high_bending)
    sim.cloths[0].materials[0].bendingori = orig_bending * new_bending_multiplier
    ...
    print("mass, stretch, bending", (new_mass_mult, new_stretch_multiplier, new_bending_multiplier))
    print(param_g.grad)
...
def do_train(cur_step,optimizer,sim,initial_states):
    ...
    while True:
        ...
        new_bending_multiplier = scale(bending_multiplier, low_bending, high_bending)
        params.append([new_stretch_multiplier, new_mass_multiplier, new_bending_multiplier])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--demo_dir', type=str, default=os.path.join('lift_real_pcls/lift_paper2/wind'))
    ...
    with open(out_path+('/log%s.txt'%timestamp),'w',buffering=1) as f:
        ...
        if args.initial_params_file: 
            ...
            initial_stretch, initial_mass, initial_bending = meteornet_preds.item()[demo_name]
            ...
            initial_bending = scale(np.clip(initial_bending, low_bending+0.1, high_bending-0.1), low_bending, high_bending, inverse=True)
            ...
            initial_bending = scale(np.mean((low_bending, high_bending)), low_bending, high_bending, inverse=True)
        initial_probs = torch.tensor([initial_stretch,initial_mass,initial_bending])
