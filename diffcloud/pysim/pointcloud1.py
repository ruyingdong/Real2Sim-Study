...
with open('conf/rigidcloth/lift/demo_diamond255.json','r') as f:
	config = json.load(f)
...
def plot_pointclouds(pcls, title="", angle=250):
...
def get_loss_per_iter(sim, epoch, sim_step, demo_dir, save, initial_states=None):
    ...
    if (save):
        ...
        plot_pointclouds([curr_pcl_cloth_only, ref_pcl, initial_pcl], title='%s/fixed_epoch%02d-%03d'%(out_path,epoch,sim_step),angle=250)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--demo_dir', type=str, default=os.path.join('lift_real_pcls/lift_paper2/brown'))
